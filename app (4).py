...
with st.sidebar:
    ...
    bom_file = st.file_uploader("BOM file (.xlsx/.xls)", type=["xlsx", "xls"], key="bom")
    req_file = st.file_uploader("Req and Stock (.xlsx/.xls)", type=["xlsx", "xls"], key="req")
    prod_file = st.file_uploader("Production Orders (.xlsx/.xls) — optional", type=["xlsx", "xls"], key="prod")
...
def read_excel_safe(file_obj, **kwargs):
    engine = get_excel_engine(file_obj)
    if hasattr(file_obj, "seek"):
        file_obj.seek(0)
    try:
        if engine:
            return pd.read_excel(file_obj, engine=engine, **kwargs)
        return pd.read_excel(file_obj, **kwargs)
    except ImportError as e:
        name = getattr(file_obj, "name", "uploaded file")
        st.error(
            f"Excel dependency missing while reading {name}. "
            f"For .xlsx install openpyxl, for .xls install xlrd. Original error: {e}"
        )
        raise
    except Exception:
        raise
...
def detect_requirement_header_row(file_obj, sheet_name="Requirement", scan_rows=20):
    raw = read_excel_safe(file_obj, sheet_name=sheet_name, header=None, nrows=scan_rows)
    best_row, best_score = 0, -1
    for i in range(len(raw)):
        cleaned = [standardize_req_header(x) for x in raw.iloc[i].tolist()]
        score = (10 if "BOM Header" in cleaned else 0) + (5 if "Alt" in cleaned else 0) + sum(1 for x in cleaned if parse_col_to_date(x))
        if score > best_score:
...
def get_ancestry_paths(component, bom):
    ...
    for _, row in comp_rows.iterrows():
        ...
        for _ in range(4):
            if current == fg:
                break
            parent_row = bom[(bom["BOM Header"] == fg) & (bom["Alt"] == alt) & (bom["Component"] == current)]
            if parent_row.empty:
                break
...
def build_dot_tree(component, paths, req_df, result_dfs, months, stock, prod_summary):
    ...
    for path in paths:
        ...
        rows = req_df[(req_df["BOM Header"] == path["fg"]) & (req_df["Alt"] == path["alt"])]
    ...
    for path in paths:
        ...
        node_attrs[fg_id] = f'label="{fg_label}" shape=box style="filled,rounded" fillcolor="#4a90d9" fontcolor=white fontsize=11'
        prev_id = fg_id
        ...
        for comp, desc, qty, sp in zip(path["path_comps"], path["path_descs"], path["path_qtys"], path["path_sp"]):
            ...
            if is_target:
                ...
                node_attrs[node_id] = f'label="{label}" shape=box style="filled,rounded" fillcolor="#27ae60" fontcolor=white fontsize=11 penwidth=2'
            elif is_ph:
                label = f"PHANTOM\\n{trunc(comp)}\\n{trunc(desc)}\\nSp.Proc: {sp}\\nQty: {qty:g}\\n(pass-through)"
                node_attrs[node_id] = f'label="{label}" shape=box style="filled,dashed" fillcolor="#f5a623" fontcolor="#333" fontsize=10'
            else:
                label = f"{trunc(comp)}\\n{trunc(desc)}\\nQty: {qty:g}  Stock: {stk:,.0f}\\nGross: {gross:,.0f}  Short: {shortage:,.0f}"
                node_attrs[node_id] = f'label="{label}" shape=box style="filled,rounded" fillcolor="#f9e79f" fontcolor="#333" fontsize=10'
...
def show_search_section(bom, req_df, months, stock, prod_summary, result_l1, result_l2, result_l3, result_l4):
    ...
    with search_col:
        search_q = st.text_input("Enter component code", placeholder="e.g. 0010748458", label_visibility="collapsed")
    if not search_q:
        return
    ...
    for p in paths:
        ...
        fg_rows.append({
            "FG code": p["fg"], "Alt": p["alt"], "BOM level": p["level"],
            "Path depth": len(p["path_comps"]), "Total demand": f"{total_demand:,.0f}",
    ...
    display_paths = paths[:12]
    if len(paths) > 12:
        st.caption(f"⚠️ Showing 12 of {len(paths)} ancestry paths for readability.")
...
def run_mrp(bom_file, req_file, prod_file):
    ...
    req = read_excel_safe(req_file, sheet_name="Requirement", header=None)
    ...
    stock_raw = read_excel_safe(req_file, sheet_name="Stock", usecols=[0, 1], header=0, names=["Component", "Stock_Qty"])
    ...
    if prod_file is not None:
        try:
            coois = read_excel_safe(prod_file)
    ...
    bom_l1 = bom[bom["Level"] == 1][["BOM Header", "Alt", "Component", "Component descriptio", "Required Qty", "Special procurement"]].copy().rename(columns={"Component": "L1_Comp", "Component descriptio": "L1_Desc", "Required Qty": "L1_Qty", "Special procurement": "L1_Ph"})
    ...
    l1_agg = l1_norm.groupby(["L1_Comp", "L1_Desc", "Month", "Month_Order"], as_index=False)["L1_Gross"].sum().rename(columns={"L1_Comp": "Component", "L1_Desc": "Desc", "L1_Gross": "Gross"})
    result_l1 = make_report(l1_agg, "Component")

    bom_l2 = bom[bom["Level"] == 2][["BOM Header", "Alt", "Parent", "Component", "Component descriptio", "Required Qty", "Special procurement"]].copy().rename(columns={"Parent": "L1_Comp", "Component": "L2_Comp", "Component descriptio": "L2_Desc", "Required Qty": "L2_Qty", "Special procurement": "L2_Ph"})
    ...
    l2_agg = l2_norm.groupby(["L2_Comp", "L2_Desc", "Month", "Month_Order"], as_index=False)["L2_Gross"].sum().rename(columns={"L2_Comp": "Component", "L2_Desc": "Desc", "L2_Gross": "Gross"})
    result_l2 = make_report(l2_agg, "Component")

    bom_l3 = bom[bom["Level"] == 3][["BOM Header", "Alt", "Parent", "Component", "Component descriptio", "Required Qty", "Special procurement"]].copy().rename(columns={"Parent": "L2_Comp", "Component": "L3_Comp", "Component descriptio": "L3_Desc", "Required Qty": "L3_Qty", "Special procurement": "L3_Ph"})
    ...
    l3_agg = l3_norm.groupby(["L3_Comp", "L3_Desc", "Month", "Month_Order"], as_index=False)["L3_Gross"].sum().rename(columns={"L3_Comp": "Component", "L3_Desc": "Desc", "L3_Gross": "Gross"})
    result_l3 = make_report(l3_agg, "Component")

    bom_l4 = bom[bom["Level"] == 4][["BOM Header", "Alt", "Parent", "Component", "Component descriptio", "Required Qty", "Special procurement"]].copy().rename(columns={"Parent": "L3_Comp", "Component": "L4_Comp", "Component descriptio": "L4_Desc", "Required Qty": "L4_Qty", "Special procurement": "L4_Ph"})
    l4 = l3.merge(bom_l4, on=["BOM Header", "Alt", "L3_Comp"], how="inner")
    l4["L4_Gross"] = l4["L3_Eff"] * l4["L4_Qty"]
    l4_agg = l4.groupby(["L4_Comp", "L4_Desc", "Month", "Month_Order"], as_index=False)["L4_Gross"].sum().rename(columns={"L4_Comp": "Component", "L4_Desc": "Desc", "L4_Gross": "Gross"})
    ...
    cols = st.columns(4)
    for col_ui, lbl, df in zip(cols, ["L1", "L2", "L3", "L4"], [result_l1, result_l2, result_l3, result_l4]):
    ...
    pivot = final_output.pivot_table(index=["Component", "Description"], columns="Month", values="Shortage", aggfunc="sum", fill_value=0).reset_index()
    ...
    pivot = pivot.merge(bom_master, on="Component", how="left").merge(stock_df, on="Component", how="left").merge(prod_summary, on="Component", how="left")
    ...
    final_cols = ["Component", "Component descri", "Procurement type", "Special procurement", "Confirmed_Qty", "Open_Production_Qty", "Stock"] + month_cols
    for c in final_cols:
    ...
    return {
        "bom": bom, "req": req, "months": months, "stock": stock,
        "prod_summary": prod_summary, "result_l1": result_l1,
        "result_l2": result_l2, "result_l3": result_l3, "result_l4": result_l4,
    }
...
st.info("For Streamlit Cloud, keep app.py, requirements.txt, and runtime.txt in the repo root.")
...
if st.session_state["mrp_results"] is not None:
    ...
    try:
        show_search_section(
            bom=r["bom"], req_df=r["req"], months=r["months"], stock=r["stock"],
            prod_summary=r["prod_summary"], result_l1=r["result_l1"], result_l2=r["result_l2"],
            result_l3=r["result_l3"], result_l4=r["result_l4"],
        )
    except Exception as e:
